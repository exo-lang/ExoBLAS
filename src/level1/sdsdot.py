...
if None not in instructions:
    exo_sdsdot_stride_1 = schedule_sdsdot_stride_1(
        C.Machine.vec_width, C.Machine.mem_type, instructions
    )
    for i in range(2):
        exo_sdsdot_stride_1 = replace(
            exo_sdsdot_stride_1, "for ii in _:_", C.Machine.convert_f32_lower_to_f64
        )
        exo_sdsdot_stride_1 = replace(
            exo_sdsdot_stride_1, "for ii in _:_", C.Machine.convert_f32_upper_to_f64
        )
else:
