...
def schedule_rot_stride_1(VEC_W, memory, instructions):
    ...
    # Stage -s
    compute_loop2 = simple_stride_1.find("for ii in _:_ #3")
    ...
    compute_loop2 = simple_stride_1.find("for ii in _:_ #6")
    simple_stride_1 = stage(simple_stride_1, [compute_loop2.body()[0].rhs().lhs()], "SNeg_Mul_X_Reg")
    compute_loop2 = simple_stride_1.find("for ii in _:_ #7")
    simple_stride_1 = stage(simple_stride_1, [compute_loop2.body()[0].rhs().rhs()], "C_Mul_Y_Reg")
    
    for buffer in ["xReg", "yReg", "cReg", "sReg", "sNegReg", "C_Mul_X_Reg", "S_Mul_Y_Reg", "SNeg_Mul_X_Reg", "C_Mul_Y_Reg"]:
        simple_stride_1 = set_memory(simple_stride_1, buffer, C.Machine.mem_type)
