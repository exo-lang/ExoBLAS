...
@proc
def sgemv(alpha: f32, beta: f32, m: size, n: size, a: f32[m, n], x: f32[n], y: f32[m]):
    for i in seq(0, m):
        result: f32
        result = 0.0
        for j in seq(0, n):
...
@proc
def sgemv_transpose(
    alpha: f32, beta: f32, n: size, m: size, a: f32[n, m], x: f32[n], y: f32[m]
):
    for i in seq(0, m):
        y[i] = beta * y[i]
        for j in seq(0, n):
...
def shared_schedule(proc):
    ...
    proc = neon_stage_expr(
        proc, "ji", "alpha_vec[_] * x[_]", "alpha_times_x", n_lifts=2
    )
...
def schedule_sgemv_transpose_on_neon():
    ...
    proc = simplify(
        stage_mem(proc, "for ii in _:_", "a[4*jo:4*jo+4, 4*io:4*io+4]", "a_vecs")
    )
...
def schedule_sgemv_on_neon():
    ...
    print("\tvectorizing alpha_times_x[_] * a[_]")
    proc = simplify(
        stage_mem(proc, "for ii in _:_", "a[4*io:4*io+4, 4*jo:4*jo+4]", "a_vecs")
    )
    proc = set_memory(proc, "a_vecs", Neon)
    proc = simplify(
        stage_mem(proc, "for i0 in _:_", "a[4*io:4*io+4, 4*jo:4*jo+4]", "a_transposed")
    )
...
def schedule_sgemv_on_neon_dot_product(i_tile=8):
    ...
    proc = simplify(
        stage_mem(
            proc, "for ji in _:_ #1", "y_partial_sums_vec[ii, 0:4]", "y_partial_sums"
        )
    )
    ...
    proc = simplify(
        stage_mem(
            proc,
            "for ii in _:_ #2",
            f"a[{i_tile}*io:{i_tile}*(io+1), 4*jo:4*jo+4]",
            "a_vec",
        )
    )
...
def schedule_sdot_stride_1_interleaved(VEC_W, INTERLEAVE_FACTOR, memory, instructions):
    ...
    stride_1 = divide_loop(
        stride_1, "for j in _:_", VEC_W * INTERLEAVE_FACTOR, ("jo", "ji"), tail="cut"
    )
    ...
    stride_1 = simplify(
        stage_mem(stride_1, "for jo in _:_", "result", "resultReg", accum=True)
    )
...
def schedule_multiple_rows_old(
    VEC_W, INTERLEAVE_FACTOR, ROW_FACTOR, memory, instructions
):
    ...
    stride_1 = divide_loop(
        stride_1, stride_1.find_loop("i"), ROW_FACTOR, ("io", "ii"), tail="cut"
    )
    ...
    stride_1 = fission(
        stride_1, stride_1.find_loop(f"ji #{INTERLEAVE_FACTOR - 1}").after()
    )
    ...
    stride_1 = divide_loop(
        stride_1, stride_1.find_loop("io"), 64, ("ioo", "ioi"), tail="cut"
    )
...
def schedule_multiple_rows(VEC_W, INTERLEAVE_FACTOR, ROW_FACTOR, memory, instructions):
    ...
    stride_1 = vectorize(stride_1, loop_cursor, VEC_W, memory, "f32")
    stride_1 = interleave_execution(
        stride_1, stride_1.find_loop("jm"), INTERLEAVE_FACTOR
    )
    stride_1 = interleave_outer_loop_with_inner_loop(
        stride_1, stride_1.find_loop("i"), stride_1.find_loop("jo"), ROW_FACTOR
    )
...
sgemv_stride_1 = schedule_multiple_rows(
    C.Machine.vec_width, 2, 4, C.Machine.mem_type, f32_instructions
)
