...
@proc
def ger_row_major_template(m: size, n: size, alpha: R, x: [R][m], y: [R][n], A: [R][m, n]):
    assert stride(A, 1) == 1
...
def schedule_interleave_ger_row_major_stride_1(VEC_W, ROW_INTERLEAVE_FACTOR, memory, instructions, precision):
    ...
    stride_1 = divide_loop(stride_1, stride_1.find_loop("i"), ROW_INTERLEAVE_FACTOR, ("io", "ii"), tail="cut")
    ...
    stride_1 = stage_mem(stride_1, stride_1.find_loop("ji"), f"x[ii + {ROW_INTERLEAVE_FACTOR} * io]", "xVal")
    stride_1 = stage_mem(stride_1, stride_1.find_loop("ji #1"), f"x[ii + m / {ROW_INTERLEAVE_FACTOR} * {ROW_INTERLEAVE_FACTOR}]", "xVal")
...
exo_sger_row_major_stride_any = rename(exo_sger_row_major_stride_any, exo_sger_row_major_stride_any.name() + "_stride_any")
...
exo_sger_row_major_stride_1 = schedule_interleave_ger_row_major_stride_1(C.Machine.vec_width, ROW_INTERLEAVE_FACTOR, C.Machine.mem_type, f32_instructions, "f32")
...
exo_dger_row_major_stride_any = rename(exo_dger_row_major_stride_any, exo_dger_row_major_stride_any.name() + "_stride_any")
...
exo_dger_row_major_stride_1 = schedule_interleave_ger_row_major_stride_1(C.Machine.vec_width // 2, ROW_INTERLEAVE_FACTOR, C.Machine.mem_type, f64_instructions, "f64")

entry_points = [exo_sger_row_major_stride_any, exo_sger_row_major_stride_1,
                exo_dger_row_major_stride_any, exo_dger_row_major_stride_1]
