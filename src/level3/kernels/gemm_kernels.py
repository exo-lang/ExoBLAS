...
class Microkernel:
    ...
    def generate_microkernel_zpad(self, machine: MachineParameters, M_r: int, N_r: int, K_blk: int, M_blk:int=32):
        base_microkernel = rename(self.sgemm_window2, f"microkernelzpad_{self.microkernel_id}")
        ...
        # Reorder and divide loops
        scheduled_microkernel = rename(base_microkernel, f"{machine.name}_microkernelzpad_{M_r}x{N_r}_{self.microkernel_id}")
        scheduled_microkernel = reorder_loops(scheduled_microkernel, 'j k')
        scheduled_microkernel = reorder_loops(scheduled_microkernel, 'i k')
        scheduled_microkernel = divide_loop(scheduled_microkernel, 'j', machine.vec_width, ['jo', 'ji'], tail='cut')

        scheduled_microkernel = autofission(scheduled_microkernel, scheduled_microkernel.find("for ji in _:_ #1").before(), n_lifts=2)

        # Create C buffer in vector mem
        c_reg_str = f'C[i, {machine.vec_width} * jo + ji]'
        scheduled_microkernel = stage_mem(scheduled_microkernel, 'C[_] += _ #0', c_reg_str, 'C_reg')
        scheduled_microkernel = set_memory(scheduled_microkernel, 'C_reg', machine.mem_type)
        scheduled_microkernel = expand_dim(scheduled_microkernel, 'C_reg', machine.vec_width, 'ji', unsafe_disable_checks=True)
        scheduled_microkernel = expand_dim(scheduled_microkernel, 'C_reg', M_blk//machine.vec_width, f'jo', unsafe_disable_checks=True)
        ...
        scheduled_microkernel = set_memory(scheduled_microkernel, 'B_vec', machine.mem_type)
        scheduled_microkernel = expand_dim(scheduled_microkernel, 'B_vec', machine.vec_width, f'ji', unsafe_disable_checks=True)
        scheduled_microkernel = expand_dim(scheduled_microkernel, 'B_vec', (M_blk // machine.vec_width), f'jo', unsafe_disable_checks=True)
        scheduled_microkernel = set_precision(scheduled_microkernel, 'B_vec', self.precision)
