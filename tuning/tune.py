...
sys.path.insert(0, os.path.abspath("/home/yuka/GPTune"))
...
def objectives(point):
    ...
    # x is from 1 to 120
    convertToBlockSize = lambda x: int(48 * round(x))
    ...
    bench = subprocess.run([BENCHSCRIPT, str(matrix_size), str(dgemm_m_blk), str(dgemm_n_blk), str(dgemm_k_blk) ], capture_output=True)
...
def constraint_1(dgemm_m_blk, matrix_size):
    return dgemm_m_blk <= int(matrix_size/48)
def constraint_2(dgemm_n_blk, matrix_size):
    return dgemm_n_blk <= int(matrix_size/48)
def constraint_3(dgemm_k_blk, matrix_size):
    return dgemm_k_blk <= int(matrix_size/48)
...
def main():
    ...
    dgemm_m_blk = Integer(1, 120, transform = "normalize", name = "dgemm_m_blk")
    dgemm_n_blk = Integer(1, 120, transform = "normalize", name = "dgemm_n_blk")
    dgemm_k_blk = Integer(1, 120, transform = "normalize", name = "dgemm_k_blk")
    
    PS = Space([ dgemm_m_blk, dgemm_n_blk, dgemm_k_blk])
    ...
    #                 "1088", "1216", "1344", "1472", "1600", "1728", "1856",
    #                 "1984", "2048"]
    giventask = [ [6144] ]# [64], [192], [256], [732], [1024], [1600], [2048] ]#, ["911"] ]
    data = Data(problem)
