...
for (int_fast32_t ioo = 0; ioo < ((((3 + n) / (4)) - 1) / (4)); ioo++) {
  ...
  var0[0] = _mm256_loadu_pd(&y.data[16 * ioo]);
  var0[1] = _mm256_loadu_pd(&y.data[4 + 16 * ioo]);
  var0[2] = _mm256_loadu_pd(&y.data[8 + 16 * ioo]);
  var0[3] = _mm256_loadu_pd(&y.data[12 + 16 * ioo]);
...
for (int_fast32_t ioi = 0; ioi < (((3 + n) / (4)) - 1) % 4; ioi++) {
  ...
  __m256d var0;
  var0 = _mm256_loadu_pd(&y.data[16 * ((((3 + n) / 4) - 1) / 4) + 4 * ioi]);
...
for (int_fast32_t io = ((3 + n) / (4)) - 1; io < ((3 + n) / (4)); io++) {
  ...
  __m256d var0;
  
       {
            ...
            __m256i prefix = _mm256_set1_epi64x((-(4 * io) + n));
            __m256i cmp = _mm256_cmpgt_epi64(prefix, indices);
            var0 = _mm256_maskload_pd(&y.data[4 * io], cmp);
       }
...
for (int_fast32_t ioo = 0; ioo < ((((7 + n) / (8)) - 1) / (4)); ioo++) {
  ...
  var0[0] = _mm256_loadu_ps(&y.data[32 * ioo]);
  var0[1] = _mm256_loadu_ps(&y.data[8 + 32 * ioo]);
  var0[2] = _mm256_loadu_ps(&y.data[16 + 32 * ioo]);
  var0[3] = _mm256_loadu_ps(&y.data[24 + 32 * ioo]);
...
for (int_fast32_t ioi = 0; ioi < (((7 + n) / (8)) - 1) % 4; ioi++) {
  ...
  __m256 var0;
  var0 = _mm256_loadu_ps(&y.data[32 * ((((7 + n) / 8) - 1) / 4) + 8 * ioi]);
  ...
  
{
    ...
    __m256i prefix = _mm256_set1_epi32((-(8 * io) + n));
    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
    var0 = _mm256_maskload_ps(&y.data[8 * io], cmp);
}
