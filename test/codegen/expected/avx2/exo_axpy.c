...
for (int_fast32_t ioi = 0; ioi < (((3 + n) / (4)) - 1) % 4; ioi++) {
  ...
  reg_3 = _mm256_add_pd(reg_2, reg_3);
  _mm256_storeu_pd(&y.data[16 * ((((3 + n) / 4) - 1) / 4) + 4 * ioi], reg_3);
}
for (int_fast32_t io = ((3 + n) / (4)) - 1; io < ((3 + n) / (4)); io++) {
  ...
  __m256d reg_3;
    ...
    __m256i prefix = _mm256_set1_epi64x((-(4 * io) + n));
    __m256i cmp = _mm256_cmpgt_epi64(prefix, indices);
    __m256d prefixed_src = _mm256_blendv_pd (_mm256_setzero_pd(), reg_2, _mm256_castsi256_pd(cmp));
    reg_3 = _mm256_add_pd(prefixed_src, reg_3);
    }
    ...
    __m256i prefix = _mm256_set1_epi64x((-(4 * io) + n));
    __m256i cmp = _mm256_cmpgt_epi64(prefix, indices);
    _mm256_maskstore_pd(&y.data[4 * io], cmp, reg_3);
    }
...
for (int_fast32_t ioo = 0; ioo < ((((3 + n) / (4)) - 1) / (4)); ioo++) {
  ...
  reg_2[0] = _mm256_loadu_pd(&y.data[16 * ioo]);
  reg_2[1] = _mm256_loadu_pd(&y.data[4 + 16 * ioo]);
  reg_2[2] = _mm256_loadu_pd(&y.data[8 + 16 * ioo]);
  reg_2[3] = _mm256_loadu_pd(&y.data[12 + 16 * ioo]);
  reg_2[0] = _mm256_fmadd_pd(reg[0], reg_1[0], reg_2[0]);
  reg_2[1] = _mm256_fmadd_pd(reg[1], reg_1[1], reg_2[1]);
  reg_2[2] = _mm256_fmadd_pd(reg[2], reg_1[2], reg_2[2]);
  reg_2[3] = _mm256_fmadd_pd(reg[3], reg_1[3], reg_2[3]);
  _mm256_storeu_pd(&y.data[16 * ioo], reg_2[0]);
  _mm256_storeu_pd(&y.data[4 + 16 * ioo], reg_2[1]);
  _mm256_storeu_pd(&y.data[8 + 16 * ioo], reg_2[2]);
  _mm256_storeu_pd(&y.data[12 + 16 * ioo], reg_2[3]);
}
for (int_fast32_t ioi = 0; ioi < (((3 + n) / (4)) - 1) % 4; ioi++) {
  __m256d reg_3;
...
for (int_fast32_t ioi = 0; ioi < (((7 + n) / (8)) - 1) % 4; ioi++) {
  ...
  reg_3 = _mm256_add_ps(reg_2, reg_3);
  _mm256_storeu_ps(&y.data[32 * ((((7 + n) / 8) - 1) / 4) + 8 * ioi], reg_3);
}
  ...
  
{
    ...
    __m256i prefix = _mm256_set1_epi32((-(8 * io) + n));
    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
    __m256 prefixed_src = _mm256_blendv_ps (_mm256_setzero_ps(), reg_2, _mm256_castsi256_ps(cmp));
    reg_3 = _mm256_add_ps(prefixed_src, reg_3);
    }
    ...
    __m256i prefix = _mm256_set1_epi32((-(8 * io) + n));
    __m256i cmp = _mm256_cmpgt_epi32(prefix, indices);
    _mm256_maskstore_ps(&y.data[8 * io], cmp, reg_3);
    }
...
for (int_fast32_t ioo = 0; ioo < ((((7 + n) / (8)) - 1) / (4)); ioo++) {
  ...
  reg_2[0] = _mm256_loadu_ps(&y.data[32 * ioo]);
  reg_2[1] = _mm256_loadu_ps(&y.data[8 + 32 * ioo]);
  reg_2[2] = _mm256_loadu_ps(&y.data[16 + 32 * ioo]);
  reg_2[3] = _mm256_loadu_ps(&y.data[24 + 32 * ioo]);
  reg_2[0] = _mm256_fmadd_ps(reg[0], reg_1[0], reg_2[0]);
  reg_2[1] = _mm256_fmadd_ps(reg[1], reg_1[1], reg_2[1]);
  reg_2[2] = _mm256_fmadd_ps(reg[2], reg_1[2], reg_2[2]);
  reg_2[3] = _mm256_fmadd_ps(reg[3], reg_1[3], reg_2[3]);
  _mm256_storeu_ps(&y.data[32 * ioo], reg_2[0]);
  _mm256_storeu_ps(&y.data[8 + 32 * ioo], reg_2[1]);
  _mm256_storeu_ps(&y.data[16 + 32 * ioo], reg_2[2]);
  _mm256_storeu_ps(&y.data[24 + 32 * ioo], reg_2[3]);
}
for (int_fast32_t ioi = 0; ioi < (((7 + n) / (8)) - 1) % 4; ioi++) {
  __m256 reg_3;
