...
for (int_fast32_t ko = 0; ko < ((K) / (192)); ko++) {
  for (int_fast32_t io = 0; io < ((M) / (4096)); io++) {
    ...
    exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,4096,N % 3072 + 0,192,(struct exo_win_2f32c){ &A.data[(4096 * io) * (A.strides[0]) + 192 * ko], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * ko) * (B.strides[0]) + 3072 * (N / 3072)], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * io) * (C.strides[0]) + 3072 * (N / 3072)], { C.strides[0], 1 } });
  }
  for (int_fast32_t jo = 0; jo < ((N) / (3072)); jo++) {
    exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,M % 4096 + 0,3072,192,(struct exo_win_2f32c){ &A.data[(4096 * (M / 4096)) * (A.strides[0]) + 192 * ko], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * ko) * (B.strides[0]) + 3072 * jo], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * (M / 4096)) * (C.strides[0]) + 3072 * jo], { C.strides[0], 1 } });
  }
  exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,M % 4096 + 0,N % 3072 + 0,192,(struct exo_win_2f32c){ &A.data[(4096 * (M / 4096)) * (A.strides[0]) + 192 * ko], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * ko) * (B.strides[0]) + 3072 * (N / 3072)], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * (M / 4096)) * (C.strides[0]) + 3072 * (N / 3072)], { C.strides[0], 1 } });
}
for (int_fast32_t io = 0; io < ((M) / (4096)); io++) {
  for (int_fast32_t jo = 0; jo < ((N) / (3072)); jo++) {
    exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,4096,3072,K % 192 + 0,(struct exo_win_2f32c){ &A.data[(4096 * io) * (A.strides[0]) + 192 * (K / 192)], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * (K / 192)) * (B.strides[0]) + 3072 * jo], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * io) * (C.strides[0]) + 3072 * jo], { C.strides[0], 1 } });
  }
  exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,4096,N % 3072 + 0,K % 192 + 0,(struct exo_win_2f32c){ &A.data[(4096 * io) * (A.strides[0]) + 192 * (K / 192)], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * (K / 192)) * (B.strides[0]) + 3072 * (N / 3072)], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * io) * (C.strides[0]) + 3072 * (N / 3072)], { C.strides[0], 1 } });
}
for (int_fast32_t jo = 0; jo < ((N) / (3072)); jo++) {
  exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,M % 4096 + 0,3072,K % 192 + 0,(struct exo_win_2f32c){ &A.data[(4096 * (M / 4096)) * (A.strides[0]) + 192 * (K / 192)], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * (K / 192)) * (B.strides[0]) + 3072 * jo], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * (M / 4096)) * (C.strides[0]) + 3072 * jo], { C.strides[0], 1 } });
}
exo_sgemm_matmul_stride_any_no_mem_sys_tiling(ctxt,M % 4096 + 0,N % 3072 + 0,K % 192 + 0,(struct exo_win_2f32c){ &A.data[(4096 * (M / 4096)) * (A.strides[0]) + 192 * (K / 192)], { A.strides[0], 1 } },(struct exo_win_2f32c){ &B.data[(192 * (K / 192)) * (B.strides[0]) + 3072 * (N / 3072)], { B.strides[0], 1 } },(struct exo_win_2f32){ &C.data[(4096 * (M / 4096)) * (C.strides[0]) + 3072 * (N / 3072)], { C.strides[0], 1 } });
}
