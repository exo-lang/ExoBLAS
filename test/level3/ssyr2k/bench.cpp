...
static void BM_SSYR2K_EXO(benchmark::State& state) {
    ...
    for (auto _: state) {
        exo_ssyr2k(CblasRowMajor, CblasLower, CblasNoTrans, n, n, &alpha, a.data(), n, a.data(), n, &beta, c.data(), n);
    }
...
BENCHMARK(BM_SSYR2K_CBLAS)->ArgNames({"n"}) -> RangeMultiplier(2) -> Range(16, 8192);
