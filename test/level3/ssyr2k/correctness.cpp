...
void test_ssyr2k(const enum CBLAS_UPLO uplo,
  ...
  cblas_ssyr2k(CblasRowMajor, uplo, transpose, n, n,  // M N
               alpha,                                 // alpha
               a.data(),
               n,  // M
               a2.data(), n, beta, c.data(),
               n  // M
  );
  if (uplo == CblasLower && transpose == CblasNoTrans && alpha == 1.0 &&
      beta == 1.0) {
    auto at = _transpose(a.data(), n, n);
    exo_ssyr2k(CblasRowMajor, uplo, transpose, n, n, &alpha, a.data(), n,
               at.data(), n, &beta, c2.data(), n);
  } else {
    exo_ssyr2k(CblasRowMajor, uplo, transpose, n, n, &alpha, a.data(), n,
               a2.data(), n, &beta, c2.data(), n);
  }
