...
static void BM_SSYMM_EXO(benchmark::State& state) {
    ...
    for (auto _: state) {
        exo_ssymm(CblasRowMajor, CblasLeft, CblasLower, n, n, &alpha, a.data(), n, b.data(), n, &beta, c.data(), n);
    }
...
BENCHMARK(BM_SSYMM_CBLAS)->ArgNames({"n"})->ArgsProduct({
      benchmark::CreateRange(32, (32 << 8), 2),
    })->ArgsProduct({
      benchmark::CreateRange(33, (32 << 8) - 1, 3),
    });
BENCHMARK(BM_SSYMM_EXO)->ArgNames({"n"})->ArgsProduct({
      benchmark::CreateRange(32, (32 << 8), 2),
    })->ArgsProduct({
      benchmark::CreateRange(33, (32 << 8) - 1, 3),
    });
