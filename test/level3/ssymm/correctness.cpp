...
void test_ssymm(const enum CBLAS_SIDE side, const enum CBLAS_UPLO uplo,
                const int n, const int m, const float alpha, const float beta) {
  std::cout << "Running ssymm test: N = " << n << ", alpha = " << alpha
            << ", beta = " << beta << ", side = " << side << ", uplo = " << uplo
            << "..." << std::endl;
  ...
  cblas_ssymm(CblasRowMajor, side, uplo, m, n, alpha, a.data(), m, b.data(), m,
              beta, c.data(), m);
  exo_ssymm(CblasRowMajor, side, uplo, m, n, &alpha, a.data(), m, b.data(), m,
            &beta, c2.data(), m);
