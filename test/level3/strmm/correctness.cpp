...
void test_strmm(const enum CBLAS_ORDER Order, const enum CBLAS_SIDE Side,
                ...
                const float alpha, const int lda, const int ldb) {
  printf(
      "Running strmm test: M = %d, N = %d, alpha = %f, lda = %d, ldb = %d, "
      "isSideLeft = %d, isUpper = %d, isTransA = %d, isDiag = %d\n",
      M, N, alpha, lda, ldb, Side == CBLAS_SIDE::CblasLeft,
      Uplo == CBLAS_UPLO::CblasUpper, TransA == CBLAS_TRANSPOSE::CblasTrans,
  ...
  exo_strmm(Order, Side, Uplo, TransA, Diag, M, N, alpha, A.data(), lda,
            B.data(), ldb);
  cblas_strmm(Order, Side, Uplo, TransA, Diag, M, N, alpha, A_expected.data(),
              lda, B_expected.data(), ldb);
              ...
              for (auto alpha : alpha_vals) {
                for (auto lda_diff : ld_diffs) {
                  for (auto ldb_diff : ld_diffs) {
                    ...
                    int lda = K + lda_diff;
                    int ldb = n + ldb_diff;
                    test_strmm(CBLAS_ORDER::CblasRowMajor, Side, Uplo, transA,
