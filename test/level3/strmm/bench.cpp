...
static void BM_cblas_strmm(benchmark::State& state) {
    ...
    const CBLAS_SIDE Side = state.range(3) == 0 ? CBLAS_SIDE::CblasLeft : CBLAS_SIDE::CblasRight;
    const CBLAS_UPLO Uplo = state.range(4) == 0 ? CBLAS_UPLO::CblasLower : CBLAS_UPLO::CblasUpper;
    const CBLAS_TRANSPOSE TransA = state.range(5) == 0 ? CBLAS_TRANSPOSE::CblasNoTrans : CBLAS_TRANSPOSE::CblasTrans;
    const CBLAS_DIAG Diag = state.range(6) == 0 ? CBLAS_DIAG::CblasNonUnit : CBLAS_DIAG::CblasUnit;
    ...
    for (auto _ : state) {
        cblas_strmm(Order, Side, Uplo, TransA, Diag, M, N, alpha, A.data(), lda, B.data(), ldb);
    }
}
                                        ...
                                        for (int alignmentB = 64; alignmentB <= 64; ++alignmentB) {
                                            for (int M = 1; M <= (1 << 10); M *= 2) {
                                                for (int N = M; N <= M; N *= 2) {
                                                    ...
                                                    int lda = K + lda_diff;
                                                    int ldb = N + ldb_diff;
                                                    b->Args({M, N, Order, Side, Uplo, TransA, Diag, alpha, lda, ldb, alignmentA, alignmentB});
                                                }
                                            }
                                        }
                                    }
...
BENCHMARK(BM_cblas_strmm)->ArgNames({"M", "N", "Order", "Side", "Uplo", "TransA", "Diag", "alpha", "lda", "ldb", "alignmentA", "alignmentB"})->Apply(CustomArgumentsPacked);
BENCHMARK(BM_exo_strmm)->ArgNames({"M", "N", "Order", "Side", "Uplo", "TransA", "Diag", "alpha", "lda", "ldb", "alignmentA", "alignmentB"})->Apply(CustomArgumentsPacked);
