...
void test_dtbmv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
    ...
    cblas_dtbmv(order, Uplo, TransA, Diag, N, k, A_expected.data(), lda, X_expected.data(), incX);
                    ...
                    for (auto lda_diff : lda_diffs) {
                        for (auto incX : incX_vals) {
                            for (auto k : K) {
                                if (k <= n - 1)
                                {
                                   test_dtbmv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n, k, k + 1 + lda_diff, incX);
                                }
                            }
                            test_dtbmv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n, n - 1, n + lda_diff, incX);
                        }
                    }
                }
            }
