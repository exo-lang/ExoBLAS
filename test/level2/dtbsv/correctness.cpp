...
void test_dtbsv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
    ...
    printf("Running dtbsv test: N = %d, K = %d, incX = %d, lda = %d, isUpper = %d, isTransA = %d, isDiag = %d\n", N, K, incX, lda, 
    ...
    cblas_dtbsv(order, Uplo, TransA, Diag, N, K, A_expected.data(), lda, X_expected.data(), incX);
                ...
                for (auto Diag: Diag_vals) {
                    for (auto lda_diff : lda_diffs) {
                        for (auto incX : incX_vals) {
                            for (auto k : K) {
                                if (k <= n - 1) {
                                    test_dtbsv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n, k, k + 1 + lda_diff, incX);
                                }
                                test_dtbsv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n, n - 1, n + lda_diff, incX);
                            }
                        }
                    }
                }
