...
static void BM_cblas_ssyr2(benchmark::State& state) {
    int N = state.range(0);
    CBLAS_ORDER order = state.range(1) == 0 ? CBLAS_ORDER::CblasRowMajor : CBLAS_ORDER::CblasColMajor;
    CBLAS_UPLO Uplo = state.range(2) == 0 ? CBLAS_UPLO::CblasUpper : CBLAS_UPLO::CblasLower;
    ...
    for (auto _ : state) {
        cblas_ssyr2(order, Uplo, N, alpha, X.data(), incX, Y.data(), incY, A.data(), lda);
    }
}
...
static void BM_exo_ssyr2(benchmark::State& state) {
    ...
    for (auto _ : state) {
        exo_ssyr2(order, Uplo, N, alpha, X.data(), incX, Y.data(), incY, A.data(), lda);
    }
}
...
BENCHMARK(BM_cblas_ssyr2)->ArgNames({"N", "order", "Uplo", "alpha", "incX", "incY", "lda", "alignmentX", "alignmentY", "alignmentA"})->Apply(CustomArgumentsPacked);
BENCHMARK(BM_exo_ssyr2)->ArgNames({"N", "order", "Uplo", "alpha", "incX", "incY", "lda", "alignmentX", "alignmentY", "alignmentA"})->Apply(CustomArgumentsPacked);
