...
void test_dsymv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
    ...
    printf("Running dsymv test: N = %d, alpha = %f, lda = %d, incX = %d, beta = %f, incY = %d, isUpper = %d\n",
                                N, alpha,
                                lda, incX,
                                beta, incY,
    ...
    cblas_dsymv(order, Uplo, N, alpha, A_expected.data(), lda, X_expected.data(), incX, beta, Y_expected.data(), incY);
                    ...
                    for (auto incX : incX_vals) {
                        for (auto beta : beta_vals) {
                            for (auto incY : incY_vals) {
                                test_dsymv(CBLAS_ORDER::CblasRowMajor,
