...
static void BM_cblas_dsymv(benchmark::State& state) {
    ...
    const double beta = state.range(6);
    ...
    for (auto _ : state) {
        cblas_dsymv(order, Uplo, N, alpha, A.data(), lda, X.data(), incX, beta, Y.data(), incY);
    }
}

static void BM_exo_dsymv(benchmark::State& state) {
    const int N = state.range(0);
    const enum CBLAS_ORDER order = state.range(1) == 0 ? CBLAS_ORDER::CblasRowMajor : CBLAS_ORDER::CblasColMajor;
    const enum CBLAS_UPLO Uplo = state.range(2) == 0 ? CBLAS_UPLO::CblasUpper : CBLAS_UPLO::CblasLower;
    const double alpha = state.range(3);
    ...
    for (auto _ : state) {
        exo_dsymv(order, Uplo, N, alpha, A.data(), lda, X.data(), incX, beta, Y.data(), incY);
    }
}
...
BENCHMARK(BM_cblas_dsymv)->ArgNames({"N", "order", "Uplo", "alpha", "lda", "incX",
                                     "beta", "incY", "alignmentA", "alignmentX",
                                     "alignmentY"})->Apply(CustomArgumentsPacked);
BENCHMARK(BM_exo_dsymv)->ArgNames({"N", "order", "Uplo", "alpha", "lda", "incX",
                                     "beta", "incY", "alignmentA", "alignmentX",
                                     "alignmentY"})->Apply(CustomArgumentsPacked);
