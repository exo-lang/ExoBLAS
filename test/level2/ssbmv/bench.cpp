...
static void BM_cblas_ssbmv(benchmark::State& state) {
    ...
    const float alpha = state.range(4);
    const int lda = state.range(5);
    const int incX = state.range(6);
    const float beta = state.range(7);
    ...
    for (auto _ : state) {
        cblas_ssbmv(order, Uplo, N, K, alpha, A.data(), lda, X.data(), incX, beta, Y.data(), incY);
    }
}

static void BM_exo_ssbmv(benchmark::State& state) {
    const int N = state.range(0);
    const int K = state.range(1);
    const enum CBLAS_ORDER order = state.range(2) == 0 ? CBLAS_ORDER::CblasRowMajor : CBLAS_ORDER::CblasColMajor;
    const enum CBLAS_UPLO Uplo = state.range(3) == 0 ? CBLAS_UPLO::CblasUpper : CBLAS_UPLO::CblasLower;
    ...
    for (auto _ : state) {
        exo_ssbmv(order, Uplo, N, K, alpha, A.data(), lda, X.data(), incX, beta, Y.data(), incY);
    }
}
...
BENCHMARK(BM_cblas_ssbmv)->ArgNames({"N", "K", "order", "Uplo", "alpha", "lda", "incX",
                                     "beta", "incY", "alignmentA", "alignmentX",
                                     "alignmentY"})->Apply(CustomArgumentsPacked);
BENCHMARK(BM_exo_ssbmv)->ArgNames({"N", "K", "order", "Uplo", "alpha", "lda", "incX",
                                     "beta", "incY", "alignmentA", "alignmentX",
                                     "alignmentY"})->Apply(CustomArgumentsPacked);
