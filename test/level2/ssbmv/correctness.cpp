...
void test_ssbmv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
    ...
    printf("Running ssbmv test: N = %d, K = %d, alpha = %f, lda = %d, incX = %d, beta = %f, incY = %d, isUpper = %d\n",
                                N, K, alpha,
                                lda, incX,
                                beta, incY,
    ...
    cblas_ssbmv(order, Uplo, N, K, alpha, A_expected.data(), lda, X_expected.data(), incX, beta, Y_expected.data(), incY);
                            ...
                            for (auto incY : incY_vals) {
                                for (auto k : K) {
                                    if (k <= n - 1) {
                                        ...
                                        test_ssbmv(CBLAS_ORDER::CblasRowMajor,
