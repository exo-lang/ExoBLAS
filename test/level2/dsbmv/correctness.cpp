...
void test_dsbmv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
                const int N, const int K, const double alpha, const int lda,
                const int incX, const double beta, const int incY) {
  printf(
      "Running dsbmv test: N = %d, K = %d, alpha = %f, lda = %d, incX = %d, "
  ...
  exo_dsbmv(order, Uplo, N, K, alpha, A.data(), lda, X.data(), incX, beta,
            Y.data(), incY);
  cblas_dsbmv(order, Uplo, N, K, alpha, A_expected.data(), lda,
              ...
              for (auto incY : incY_vals) {
                for (auto k : K) {
                  if (k <= n - 1) {
                    ...
                    test_dsbmv(CBLAS_ORDER::CblasRowMajor, Uplo, n, k, alpha,
                               lda, incX, beta, incY);
                  }
                }
                int lda = n + lda_diff;
                test_dsbmv(CBLAS_ORDER::CblasRowMajor, Uplo, n, n - 1, alpha,
                           lda, incX, beta, incY);
              }
            }
          }
