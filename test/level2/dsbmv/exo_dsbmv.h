...
void exo_dsbmv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
    ...
    if (Uplo == CBLAS_UPLO::CblasUpper) {
        if (incX == 1 && incY == 1) {
            exo_dsbmv_raw_major_Upper_stride_1(nullptr, N, K, &alpha,
                exo_win_2f64c{.data = A, .strides = {lda, 1}},
                exo_win_1f64c{.data = X, .strides = {incX}}, 
                exo_win_1f64{.data = Y, .strides = {incY}} 
                );
        } else {
            exo_dsbmv_raw_major_Upper_stride_any(nullptr, N, K, &alpha,
                exo_win_2f64c{.data = A, .strides = {lda, 1}},
                exo_win_1f64c{.data = X, .strides = {incX}}, 
                exo_win_1f64{.data = Y, .strides = {incY}}
                );
        ...
        if (incX == 1 && incY == 1) {
            exo_dsbmv_raw_major_Lower_stride_1(nullptr, N, K, &alpha,
                exo_win_2f64c{.data = A, .strides = {lda, 1}},
                exo_win_1f64c{.data = X, .strides = {incX}}, 
                exo_win_1f64{.data = Y, .strides = {incY}}
                );
        } else {
            exo_dsbmv_raw_major_Lower_stride_any(nullptr, N, K, &alpha,
                exo_win_2f64c{.data = A, .strides = {lda, 1}},
                exo_win_1f64c{.data = X, .strides = {incX}}, 
                exo_win_1f64{.data = Y, .strides = {incY}}
                );
