...
static void BM_cblas_dsyr2(benchmark::State& state) {
    ...
    for (auto _ : state) {
        cblas_dsyr2(order, Uplo, N, alpha, X.data(), incX, Y.data(), incY, A.data(), lda);
    }
}
...
static void BM_exo_dsyr2(benchmark::State& state) {
    ...
    for (auto _ : state) {
        exo_dsyr2(order, Uplo, N, alpha, X.data(), incX, Y.data(), incY, A.data(), lda);
    }
}
...
BENCHMARK(BM_cblas_dsyr2)->ArgNames({"N", "order", "Uplo", "alpha", "incX", "incY", "lda", "alignmentX", "alignmentY", "alignmentA"})->Apply(CustomArgumentsPacked);
BENCHMARK(BM_exo_dsyr2)->ArgNames({"N", "order", "Uplo", "alpha", "incX", "incY", "lda", "alignmentX", "alignmentY", "alignmentA"})->Apply(CustomArgumentsPacked);
