...
void test_dsyr2(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo, int N, double alpha, int incX, int incY, int lda) {
    printf("Running dsyr2 test: N = %d, alpha = %f, incX = %d, incY = %d, lda = %d, isUpper = %d\n", N, alpha, incX, incY, lda, Uplo == CBLAS_UPLO::CblasUpper);
    ...
    exo_dsyr2(CBLAS_ORDER::CblasRowMajor, Uplo, N, alpha, X.data(), incX, Y.data(), incY, A.data(), lda);
    cblas_dsyr2(CBLAS_ORDER::CblasRowMajor, Uplo, N, alpha, X_expected.data(), incX, Y_expected.data(), incY, A_expected.data(), lda);
                ...
                for (auto incX : incX_vals) {
                    for (auto incY : incY_vals) {
                        for (auto lda_diff : lda_diffs) {
                            int lda = n + lda_diff;
                            test_dsyr2(CBLAS_ORDER::CblasRowMajor, Uplo, n, alpha, incX, incY, lda);
                        }
                    }
                }
            }
