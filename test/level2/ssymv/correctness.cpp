...
void test_ssymv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
                const int N, const float alpha, const int lda, const int incX,
                const float beta, const int incY) {
  printf(
      "Running ssymv test: N = %d, alpha = %f, lda = %d, incX = %d, beta = %f, "
      "incY = %d, isUpper = %d\n",
  ...
  exo_ssymv(order, Uplo, N, alpha, A.data(), lda, X.data(), incX, beta,
            Y.data(), incY);
  cblas_ssymv(order, Uplo, N, alpha, A_expected.data(), lda, X_expected.data(),
          ...
          for (auto incX : incX_vals) {
            for (auto beta : beta_vals) {
              for (auto incY : incY_vals) {
                test_ssymv(CBLAS_ORDER::CblasRowMajor, Uplo, n, alpha, lda,
                           incX, beta, incY);
              }
            }
          }
