...
static void BM_cblas_dgemv(benchmark::State &state) {
  ...
  for (auto _ : state) {
    cblas_dgemv(order, TransA, N, N, alpha, A.data(), lda, X.data(), incX, beta,
                Y.data(), incY);
  }
}

static void BM_exo_dgemv(benchmark::State &state) {
  const int N = state.range(0);
  const int M = N;
  const enum CBLAS_ORDER order = state.range(1) == 0
                                     ? CBLAS_ORDER::CblasRowMajor
  ...
  for (auto _ : state) {
    exo_dgemv(order, TransA, N, N, alpha, A.data(), lda, X.data(), incX, beta,
              Y.data(), incY);
  }
}
