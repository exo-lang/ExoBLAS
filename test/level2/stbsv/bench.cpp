...
static void BM_cblas_stbsv(benchmark::State& state) {
    ...
    for (auto _ : state) {
        cblas_stbsv(order, Uplo, TransA, Diag, N, K, A.data(), lda, X.data(), incX);
    }
}

static void BM_exo_stbsv(benchmark::State& state) {
    const int N = state.range(0);
    const int K = state.range(1);
    const CBLAS_ORDER order = state.range(2) == 0 ? CBLAS_ORDER::CblasRowMajor : CBLAS_ORDER::CblasColMajor;
    const CBLAS_UPLO Uplo = state.range(3) == 0 ? CBLAS_UPLO::CblasLower : CBLAS_UPLO::CblasUpper;
...
BENCHMARK(BM_exo_stbsv)->ArgNames({"N", "K", "order", "Uplo", "TransA", "Diag", "lda", "incX", "alignmentX", "alignmentA"})->Apply(CustomArgumentsPacked);
