...
void test_strmv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
    ...
    printf("Running trmv test: N = %d, incX = %d, lda = %d, isUpper = %d, isTransA = %d, isDiag = %d\n", N, incX, lda, 
    ...
    cblas_strmv(order, Uplo, TransA, Diag, N, A_expected.data(), lda, X_expected.data(), incX);
                ...
                for (auto Diag: Diag_vals) {
                    for (auto lda_diff : lda_diffs) {
                        for (auto incX : incX_vals) {
                            test_strmv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n, n + lda_diff, incX);
                        }
                    }
                }
            }
