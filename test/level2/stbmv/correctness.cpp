...
void test_stbmv(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
  ...
  printf(
      "Running stbmv test: N = %d, k = %d, incX = %d, lda = %d, isUpper = %d, "
      "isTransA = %d, isDiag = %d\n",
  ...
  cblas_stbmv(order, Uplo, TransA, Diag, N, k, A_expected.data(), lda,
        ...
        for (auto Diag : Diag_vals) {
          for (auto lda_diff : lda_diffs) {
            for (auto incX : incX_vals) {
              for (auto k : K) {
                if (k <= n - 1) {
                  test_stbmv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n,
                             k, k + 1 + lda_diff, incX);
                }
              }
              test_stbmv(CBLAS_ORDER::CblasRowMajor, Uplo, transA, Diag, n,
