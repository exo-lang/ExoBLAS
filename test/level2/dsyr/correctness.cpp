...
void test_dsyr(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
                ...
                const int incX, const int lda) {
    printf("Running dsyr test: N = %d, alpha = %f, incX = %d, lda = %d, isUpper = %d\n", N, alpha, incX, lda, Uplo == CBLAS_UPLO::CblasUpper);
    ...
    cblas_dsyr(order, Uplo, N, alpha, X_expected.data(), incX, A_expected.data(), lda);
...
int main () {
    std::vector<CBLAS_UPLO> Uplo_values {CBLAS_UPLO::CblasUpper, CBLAS_UPLO::CblasLower};
