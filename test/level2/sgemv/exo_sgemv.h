...
void exo_sgemv(const enum CBLAS_ORDER order, const enum CBLAS_TRANSPOSE TransA,
  ...
  if (TransA == CBLAS_TRANSPOSE::CblasNoTrans) {
    if (incX == 1 && incY == 1) {
      exo_sgemv_rm_nt_stride_1(nullptr, M, N, &alpha, &beta,
      ...
      exo_sgemv_rm_nt_stride_any(nullptr, M, N, &alpha, &beta,
                                 exo_win_2f32c{.data = A, .strides = {lda, 1}},
                                 exo_win_1f32c{.data = X, .strides = {incX}},
                                 exo_win_1f32{.data = Y, .strides = {incY}});
    }
    ...
    if (incX == 1 && incY == 1) {
      exo_sgemv_rm_t_stride_1(nullptr, N, M, &alpha, &beta,
                              exo_win_2f32c{.data = A, .strides = {lda, 1}},
                              exo_win_1f32c{.data = X, .strides = {incX}},
                              exo_win_1f32{.data = Y, .strides = {incY}});
    } else {
      ...
      exo_sgemv_rm_t_stride_any(nullptr, N, M, &alpha, &beta,
                                exo_win_2f32c{.data = A, .strides = {lda, 1}},
                                exo_win_1f32c{.data = X, .strides = {incX}},
                                exo_win_1f32{.data = Y, .strides = {incY}});
    }
