...
static void BM_cblas_strsv(benchmark::State &state) {
  ...
  for (auto _ : state) {
    cblas_strsv(order, Uplo, TransA, Diag, N, A.data(), lda, X.data(), incX);
  }
}

static void BM_exo_strsv(benchmark::State &state) {
  const int N = state.range(0);
  const CBLAS_ORDER order = state.range(1) == 0 ? CBLAS_ORDER::CblasRowMajor
                                                : CBLAS_ORDER::CblasColMajor;
  const CBLAS_UPLO Uplo =
